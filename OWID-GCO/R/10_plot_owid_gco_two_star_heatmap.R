# Plot the two-star OWID-GCO cross-database heatmap from post-run data.
...
matrix_file <- "data/cross_database/owid_gco/matrices/two_star/cancer_vs_OWID_partial_correlation_GDPcontrolled_min30_ISOmatched.csv"
topic_file <- "data/cross_database/owid_gco/metadata/cancer_vs_OWID_two_star_heatmap_item_topic_mapping_used.csv"
out_dir <- "outputs/figures"
min_non_missing <- 30

dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

pcorr_mat <- as.matrix(read.csv(matrix_file, row.names = 1, check.names = FALSE))
mode(pcorr_mat) <- "numeric"

old_names <- rownames(pcorr_mat)
new_names <- standardize_cancer_names(old_names)
rownames(pcorr_mat) <- new_names
write.csv(
...
names(topic_vec) <- topic_map$partial_correlation_matrix_column

keep_items <- colSums(!is.na(pcorr_mat)) >= min_non_missing
pcorr_filt <- pcorr_mat[, keep_items, drop = FALSE]
common_items <- intersect(colnames(pcorr_filt), names(topic_vec))
pcorr_filt <- pcorr_filt[, common_items, drop = FALSE]
...
pcorr_plot <- pcorr_filt[, col_order, drop = FALSE]
...
row_hc <- hclust(dist_fun_rows(pcorr_plot), method = "average")
...
png(file.path(out_dir, "cancer_vs_OWID_two_star_heatmap_GDPcontrolled.png"), width = 6000, height = 2600, res = 300)
...
pdf(file.path(out_dir, "cancer_vs_OWID_two_star_heatmap_GDPcontrolled.pdf"), width = 20, height = 8.8)
draw(ht, heatmap_legend_side = "right", annotation_legend_list = list(lgd_topic), annotation_legend_side = "bottom", merge_legends = FALSE, padding = unit(c(2, 2, 10, 2), "mm"))
dev.off()

message("Two-star OWID-GCO heatmap completed.")
