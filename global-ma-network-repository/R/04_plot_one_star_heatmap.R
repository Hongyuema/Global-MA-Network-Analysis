...
input_matrix_file <- get_arg("input_matrix", file.path("data", "matrices", "one_star", paste0("OWID_item_correlation_matrix_", year, "_min", min_links, ".csv")))
...
output_prefix <- get_arg("output_prefix", file.path(output_dir, paste0("OWID_one_star_heatmap_", year, "_final")))
...
mapping_used <- data.frame(
  correlation_matrix_column = colnames(corr_plot),
  mapped_topic = topic_filt,
  stringsAsFactors = FALSE
)
mapping_file <- file.path(table_dir, paste0("OWID_one_star_heatmap_", year, "_item_topic_mapping_final.csv"))
